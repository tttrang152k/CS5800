...
class RateAmountAscSortStrategyTest {

    @Test
    void test_sort_sorts_by_rate_amount_ascending() {
        RateAmountAscSortStrategy strategy = new RateAmountAscSortStrategy();
        ...
        assertEquals("L-2", sorted.get(0).getId()); // 100
        assertEquals("L-3", sorted.get(1).getId()); // 200
        assertEquals("L-1", sorted.get(2).getId()); // 300
    }
}
